...
while True:
    text  = input('swan > ')
    result,error=swan.run('<hata>',text)
    
    if error: print(error.as_string())
    else: print(result)
    
#girilen metni işleyerek bir sonuç üretir Bu işlev text adlı parametreyle birlikte çağrılır ve bir sonuç ve bir hata nesnesi varsa döndürür
